(function (global) {
  ...
  const safeString = (value, fallback = '') => { const s = asText(value); return s || fallback; };
  const hasValue = (value) => value !== null && value !== undefined && String(value).trim() !== '';
  const safeArray = (value) => Array.isArray(value) ? value : [];
  const safeObject = (value) => (value && typeof value === 'object' && !Array.isArray(value)) ? value : {};
  const uniq = (arr) => [...new Set(safeArray(arr).filter(Boolean))];
  const toNum = (value, fallback = null) => { const n = Number(value); return Number.isFinite(n) ? n : fallback; };
  ...
  const toTeamId = (value) => { const v = asText(value); if (!v) return ''; return /^TEAM-/i.test(v) ? v : `TEAM-${slug(v)}`; };
  const toFunctionId = (value) => { const v = asText(value); if (!v) return ''; return /^CF-/i.test(v) ? v : `CF-${slug(v)}`; };
  const toRiskId = (value) => { const v = asText(value); if (!v) return ''; return /^RISK-/i.test(v) ? v : `RISK-${slug(v)}`; };
  ...
  const set = (key, value) => { localStorage.setItem(key, JSON.stringify(value)); return value; };
  const update = (key, updaterFn) => { const next = updaterFn(get(key, null)); return set(key, next); };
  ...
  const normalizeBiaRecord = (record = {}, context = {}) => {
    ...
    return {
      ...
      functionId: core.id, functionName: core.name,
  ...
  const normalizeIncidentRecord = (record = {}, idx = 0) => {
    ...
    return {
      ...
      relatedTeamNames, relatedFunctionNames, relatedRiskNames,
  ...
  const getAllTeams = () => {
    ...
    return org.divisions.flatMap((d) => safeArray(d.teams).map((team) => normalizeTeam({ ...team, division: d.name })));
  };
  const getTeamById = (teamId) => {
    const id = asText(teamId); if (!id) return null;
  ...
  const writeServices  = (services) => { const n = safeArray(services).map((row) => normalizeService(row)); set(KEY_SERVICE_REGISTRY, n); return n; };
  const getServiceById = (serviceId) => { const id = asText(serviceId); if (!id) return null; return getAllServices().find((s) => asText(s.id) === id) || null; };

  const getAllCoreFunctions        = () => readCoreFunctions();
  const getCoreFunctionById        = (functionId) => { const id = asText(functionId); if (!id) return null; return readCoreFunctions().find((fn) => asText(fn.id) === id) || null; };
  const getCoreFunctionsByTeamId   = (teamId) => { const id = asText(teamId); if (!id) return []; return readCoreFunctions().filter((fn) => asText(fn.teamId) === id); };
  ...
  const readRawCollection     = (logicalName) => { const key = resolveStorageKey(logicalName); return key ? get(key, null) : null; };
  const writeRawCollection    = (logicalName, value) => { const key = resolveStorageKey(logicalName); if (key) set(key, value); return value; };
  const mergeLegacyCollections = (logicalName) => {
    ...
    safeArray(contract.legacyKeys).forEach((k) => { const v = get(k, null); if (v !== null) merged.push(v); });
    return merged;
  };
  ...
  const normalizeCollection = (logicalName, items) => {
    ...
    if (logicalName === 'incidents')     return safeArray(items).map((row, i) => normalizeIncidentRecord(row, i));
    ...
    if (logicalName === 'focusTarget') {
      const s = safeObject(items);
      return {
        source: asText(s.source), incidentId: asText(s.incidentId || s.id),
        assetId: asText(s.assetId), assetType: asText(s.assetType).toUpperCase(),
        assetCollection: asText(s.assetCollection).toLowerCase(), serviceName: asText(s.serviceName),
        relatedTeamIds:     safeIdArray(s.relatedTeamIds     || [], 'team'),
        relatedFunctionIds: safeIdArray(s.relatedFunctionIds || [], 'function'),
        relatedRiskIds:     safeIdArray(s.relatedRiskIds     || [], 'risk'),
  ...
  const writeCanonicalCollection = (logicalName, items) => {
    ...
    if (logicalName === 'incidents')     return set(KEY_INCIDENTS_UNIFIED, safeArray(items).map((row, i) => normalizeIncidentRecord(row, i)));
  ...
  const upsertCollectionItem = (logicalName, item) => {
    const next = upsertById(safeArray(readCanonicalCollection(logicalName)), item);
    return writeCanonicalCollection(logicalName, next);
  };
  const removeCollectionItem = (logicalName, id) => {
    const key = asText(id);
    const next = safeArray(readCanonicalCollection(logicalName)).filter((row) => asText(row?.id) !== key);
    return writeCanonicalCollection(logicalName, next);
  };

  // ── Incident execution log ────────────────────────────────────
  const getIncidentExecutionLog = (incidentId) => {
    const id   = asText(incidentId); if (!id) return [];
  ...
  const syncRiskWithBia = (biaRecordsInput = null) => {
    const biaRecords     = safeArray(biaRecordsInput?.length ? biaRecordsInput : readBiaRecords()).map((row) => normalizeBiaRecord(row));
    ...
    biaRecords.forEach((bia) => {
      ...
      const merged = normalizeRiskRecord({
        ...prev, teamId: bia.teamId, teamName: bia.teamName,
        functionId: bia.functionId, functionName: bia.functionName, serviceName: bia.serviceName,
        impact: bia.overallImpact, likelihood: clamp(toNum(prev?.likelihood, 2), 1, 5),
        threat: asText(prev?.threat || ''), source: 'BIA_AUTO', sourceRefId, stale: false
      });
    ...
    existingRisks
      ...
      .forEach((risk) => next.push(normalizeRiskRecord({ ...risk, stale: true, source: 'BIA_AUTO' })));
  ...
  const readIncidentRecords = () => {
    ...
    safeArray(get(KEY_INCIDENT_EXECUTION, [])).forEach((row) => {
      if (!row || typeof row !== 'object') return;
      rows.push({ incidentId: row.incidentId || row.id || row.incidentRef || '', status: row.status || row.currentStatus || '', severity: row.severity || row.level || '', serviceName: row.serviceName || row.service || '', startedAt: row.startedAt || row.createdAt || '', endedAt: row.endedAt || '' });
    });
    const normalized = rows.map((row, idx) => normalizeIncidentRecord(row, idx)).filter((row) => hasValue(row.id));
    const dedup = new Map();
    normalized.forEach((row) => { const prev = dedup.get(row.id); dedup.set(row.id, prev ? { ...prev, ...row, raw: { ...safeObject(prev.raw), ...safeObject(row.raw) } } : row); });
    return [...dedup.values()];
  };
  ...
  const getIncidentById = (incidentId) => {
    const id = asText(incidentId); if (!id) return null;
  ...
  const resolveIncidentFunctions = (incident) => {
    ...
    row.relatedFunctionNames.forEach((name) => { const id = toFunctionId(name); if (!fnMap.has(id)) fnMap.set(id, { functionName: name, teamId: '', teamName: '', serviceName: '' }); });
    return uniqueIds(row.relatedFunctionIds).map((id) => ({ id, name: asText(fnMap.get(id)?.functionName || id.replace(/^CF-/, '')), teamId: asText(fnMap.get(id)?.teamId || ''), teamName: asText(fnMap.get(id)?.teamName || ''), serviceName: asText(fnMap.get(id)?.serviceName || '') }));
  };
  const resolveIncidentRisks = (incident) => {
    const row     = normalizeIncidentRecord(incident || {});
    const riskMap = new Map(readRiskRecords().map((r) => [asText(r.id), r]));
    row.relatedRiskNames.forEach((name) => { const id = toRiskId(name); if (!riskMap.has(id)) riskMap.set(id, { threat: name, riskScore: 0, teamId: '', functionId: '' }); });
    return uniqueIds(row.relatedRiskIds).map((id) => ({ id, threat: asText(riskMap.get(id)?.threat || id.replace(/^RISK-/, '')), riskScore: toNum(riskMap.get(id)?.riskScore, 0) || 0, teamId: asText(riskMap.get(id)?.teamId || ''), functionId: asText(riskMap.get(id)?.functionId || '') }));
  };
  const normalizeIncidentContext = (incident) => {
    const row = normalizeIncidentRecord(incident || {});
    const teams = resolveIncidentTeams(row); const functions = resolveIncidentFunctions(row); const risks = resolveIncidentRisks(row);
    const riskTags = uniqueIds([...risks.map((r) => asText(r.threat).toLowerCase()), ...row.relatedRiskNames.map((name) => asText(name).toLowerCase())].filter(Boolean));
    return { incidentId: row.id, title: asText(row.title || row.raw?.title || ''), serviceName: asText(row.serviceName || ''), severity: asText(row.severity || ''), status: asText(row.status || ''), relatedTeamIds: uniqueIds(teams.map((t) => asText(t.id))), relatedFunctionIds: uniqueIds(functions.map((f) => asText(f.id))), relatedRiskIds: uniqueIds(risks.map((r) => asText(r.id))), relatedTeamNames: uniqueIds(teams.map((t) => asText(t.name)).filter(Boolean)), relatedFunctionNames: uniqueIds(functions.map((f) => asText(f.name)).filter(Boolean)), relatedRiskTags: riskTags };
  };
  const resolveIncidentContext = (incident) => {
    const base = normalizeIncidentRecord(incident || {});
    const teams = resolveIncidentTeams(base); const functions = resolveIncidentFunctions(base); const risks = resolveIncidentRisks(base);
  ...
  const getFocusTarget = () => {
    const src = safeObject(get(KEY_FOCUS_TARGET, {}));
    return { source: asText(src.source), incidentId: asText(src.incidentId || src.id), assetId: asText(src.assetId), assetType: asText(src.assetType).toUpperCase(), assetCollection: asText(src.assetCollection).toLowerCase(), serviceName: asText(src.serviceName), relatedTeamIds: safeIdArray(src.relatedTeamIds || [], 'team'), relatedFunctionIds: safeIdArray(src.relatedFunctionIds || [], 'function'), relatedRiskIds: safeIdArray(src.relatedRiskIds || [], 'risk'), createdAt: asText(src.createdAt) };
  };
  const setFocusTarget = (payload = {}) => {
    const target = getFocusTarget(); const src = safeObject(payload);
    const next = { ...target, source: asText(src.source || target.source), incidentId: asText(src.incidentId || src.id || target.incidentId), assetId: asText(src.assetId || target.assetId), assetType: asText(src.assetType || target.assetType).toUpperCase(), assetCollection: asText(src.assetCollection || target.assetCollection).toLowerCase(), serviceName: asText(src.serviceName || target.serviceName), relatedTeamIds: safeIdArray(src.relatedTeamIds || target.relatedTeamIds || [], 'team'), relatedFunctionIds: safeIdArray(src.relatedFunctionIds || target.relatedFunctionIds || [], 'function'), relatedRiskIds: safeIdArray(src.relatedRiskIds || target.relatedRiskIds || [], 'risk'), createdAt: asText(src.createdAt || nowISO()) };
  ...
  const getAllEvidence = () => {
    const merged = [...safeArray(get(KEY_EVIDENCE_ITEMS, [])), ...safeArray(get(KEY_EVIDENCE, []))].map((row) => normalizeEvidenceRecord(row));
    const byId = new Map(); merged.forEach((row) => byId.set(row.id, { ...(byId.get(row.id) || {}), ...row }));
    return [...byId.values()];
  };
  const writeEvidenceItems = (items) => {
    const normalized = safeArray(items).map((row) => normalizeEvidenceRecord(row));
    set(KEY_EVIDENCE_ITEMS, normalized); set(KEY_EVIDENCE, normalized); return normalized;
  };
  const getAllCapaItems = () => {
    const merged = [...safeArray(get(KEY_CAPA_ITEMS, [])), ...safeArray(get(KEY_ACTION_ITEMS, [])), ...safeArray(get(KEY_CAPA, []))].map((row) => normalizeCapaRecord(row));
    const byId = new Map(); merged.forEach((row) => byId.set(row.id, { ...(byId.get(row.id) || {}), ...row }));
    return [...byId.values()];
  };
  const writeCapaItems = (items) => {
    const normalized = safeArray(items).map((row) => normalizeCapaRecord(row));
    set(KEY_CAPA_ITEMS, normalized); set(KEY_ACTION_ITEMS, normalized); set(KEY_CAPA, normalized); return normalized;
  };
  ...
  const getRecentClosedIncidents = () => readIncidentRecords().filter((row) => hasValue(row.endedAt) || ['종료','resolved','closed','completed','ended'].includes(asText(row.status).toLowerCase())).sort((a, b) => new Date(b.endedAt || b.startedAt || 0).getTime() - new Date(a.endedAt || a.startedAt || 0).getTime());
  ...
  const validateCollectionShape = (logicalName) => {
    ...
    if (typeof raw === 'object') { malformed.type = 'object'; return malformed; }
    malformed.type = typeof raw; malformed.malformedCount = 1; return malformed;
  };
  const countMalformedItems    = (logicalName) => validateCollectionShape(logicalName).malformedCount;
  const summarizeStorageUsage  = () => Object.keys(STORAGE_KEY_CONTRACT).map((name) => { const c = STORAGE_KEY_CONTRACT[name]; return { logicalName: name, canonicalKey: c.canonicalKey, hasCanonical: localStorage.getItem(c.canonicalKey) !== null, legacyUsed: safeArray(c.legacyKeys).filter((k) => localStorage.getItem(k) !== null) }; });
  const getStorageHealthReport = () => ({ generatedAt: nowISO(), collections: Object.keys(STORAGE_KEY_CONTRACT).map((name) => ({ ...validateCollectionShape(name), contract: STORAGE_KEY_CONTRACT[name] })), usage: summarizeStorageUsage() });
  ...
  const assertNoDuplicateAutoItems = () => {
    ...
    const uniqueEvidence = new Set(evidence.map(keyOf)); const uniqueCapa = new Set(capa.map(keyOf));
    return { pass: uniqueEvidence.size === evidence.length && uniqueCapa.size === capa.length, evidence: { total: evidence.length, unique: uniqueEvidence.size }, capa: { total: capa.length, unique: uniqueCapa.size } };
  };
  const assertSelectedIncidentCompat = () => {
    ...
    return { pass: !canonical && !legacyId ? true : (canonical ? !legacyId || canonical === legacyId : Boolean(legacyId)), canonicalId: canonical, legacyId };
  };
  const assertStorageFailSoft = () => {
    const keys = Object.values(STORAGE_KEY_CONTRACT).flatMap((c) => [c.canonicalKey, ...safeArray(c.legacyKeys)]);
    const malformedKeys = []; keys.forEach((key) => { const raw = localStorage.getItem(key); if (raw === null) return; try { JSON.parse(raw); } catch { malformedKeys.push(key); } });
    return { pass: true, malformedKeys };
  };
  const getRegressionSnapshot = () => {
    const snap = getOpsStatusSnapshot();
    return { generatedAt: nowISO(), counts: { activeIncidents: snap.activeIncidents.length, pendingEvidence: snap.pendingEvidence.length, openCapa: snap.openCapaItems.length, priorityRisks: snap.priorityRisks.length }, selectedIncident: assertSelectedIncidentCompat(), duplicates: assertNoDuplicateAutoItems(), storageHealth: getStorageHealthReport() };
  };
  ...
  // ── Export ────────────────────────────────────────────────────
  global.DataStore = {
    KEY_SERVICE_REGISTRY, KEY_INCIDENTS, KEY_INCIDENT_LOGS, KEY_EVIDENCE, KEY_CAPA,
    KEY_THEME, KEY_DEMO_MODE, KEY_RTO_THRESHOLD, KEY_ORG_REGISTRY, KEY_EOP_ROLE_MAPPING,
    KEY_BCP_MODE, KEY_CORE_FUNCTIONS, KEY_BIA_DATA, KEY_RISK_ASSESSMENT,
    KEY_INCIDENTS_UNIFIED, KEY_INCIDENT_EXECUTION, KEY_SELECTED_INCIDENT_ID,
    KEY_SELECTED_INCIDENT, KEY_FOCUS_TARGET, KEY_EVIDENCE_ITEMS, KEY_ACTION_ITEMS, KEY_CAPA_ITEMS,
    get, set, update, remove, reset,
    nowISO, uid,
    safeArray, safeObject, safeString, safeId, safeParseJson, stableSlug, safeTimestamp,
    upsertById,
    normalizeTeam, normalizeService, normalizeCoreFunction, normalizeBiaRecord,
    normalizeRiskRecord, normalizeIncidentRecord,
    safeIdArray, uniqueIds, safeStatus,
    readCoreFunctions, writeCoreFunctions,
    getAllTeams, getTeamById,
    getAllServices, writeServices, getServiceById,
    getAllCoreFunctions, getCoreFunctionById, getCoreFunctionsByTeamId,
    getOrgRegistry, writeOrgRegistry, saveOrgRegistry,
    getEopRoleAssignments,
    getStorageContract, resolveStorageKey,
    readRawCollection, writeRawCollection, mergeLegacyCollections,
    normalizeCollection, readCanonicalCollection, writeCanonicalCollection,
    upsertCollectionItem, removeCollectionItem,
    validateCollectionShape, countMalformedItems, summarizeStorageUsage, getStorageHealthReport,
    compareCountVsList, assertNoDuplicateAutoItems, assertSelectedIncidentCompat,
    assertStorageFailSoft, getRegressionSnapshot, runSmokeChecks,
    readBiaRecords, writeBiaRecords, getAllBiaRecords, saveBiaRecords,
    readRiskRecords, writeRiskRecords, getAllRiskRecords, saveRiskRecords,
    syncRiskWithBia,
    readIncidentRecords, getAllIncidents, saveIncidents, getIncidentById,
    getSelectedIncidentId, normalizeIncidentContext, setSelectedIncidentId,
    getFocusTarget, setFocusTarget, clearFocusTarget,
    getIncidentExecutionLog, saveIncidentExecutionLog,
    resolveIncidentTeams, resolveIncidentFunctions, resolveIncidentRisks, resolveIncidentContext,
    normalizeEvidenceRecord, normalizeCapaRecord,
    getAllEvidence, writeEvidenceItems, saveEvidence,
    getAllCapaItems, writeCapaItems, saveCapaItems,
    getPendingEvidence, getOpenCapaItems, getActiveIncidents, getRecentClosedIncidents,
